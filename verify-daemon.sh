#!/bin/bash
# PassLock Daemon - Installation Verification Script
...
if [ "$OS" = "linux" ]; then
    echo "🛠️  Checking Linux dependencies..."
    ...
    if [ -f ~/.config/systemd/user/passlock.service ]; then
        ...
        if systemctl --user is-active passlock &> /dev/null; then
            ...
            echo "   ℹ️  Service not running (start manually)"
        fi
    else
...
if [ "$OS" = "macos" ]; then
    echo "🛠️  Checking macOS dependencies..."
...
if command -v cargo &> /dev/null; then
    ...
    echo "   ⚠️  Rust not found (only needed for building)"
fi
echo ""
...
if [ $ERRORS -eq 0 ] && [ $WARNINGS -eq 0 ]; then
    ...
    echo ""
    echo "🚀 You're ready to use PassLock daemon!"
    echo ""
    echo "Quick start:"
