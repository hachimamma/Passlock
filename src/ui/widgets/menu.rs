...
pub fn draw_main_menu(f: &mut Frame, size: Rect, app: &App) {
    ...
    if let Some(ref vault) = app.vault {
        ...
        let info_lines = vec![
            ...
            Line::from(""),
            Line::from(vec![
                Span::styled("Build: v", Style::default().fg(app.theme.gray()).add_modifier(Modifier::BOLD)),
                Span::styled(env!("CARGO_PKG_VERSION"), Style::default().fg(app.theme.yellow())),
            ]),
            Line::from(vec![
                Span::styled("Cipher: ", Style::default().fg(app.theme.gray()).add_modifier(Modifier::BOLD)),
                Span::styled(cipher, Style::default().fg(app.theme.purple())),
            ]),
            Line::from(vec![
                Span::styled("Entries: ", Style::default().fg(app.theme.gray()).add_modifier(Modifier::BOLD)),
                Span::styled(format!("{}", total), Style::default().fg(app.theme.green())),
            ]),
            Line::from(vec![
                Span::styled("Stats: ", Style::default().fg(app.theme.gray()).add_modifier(Modifier::BOLD)),
                Span::styled(format!("{} 2FA │ {} tags", with_2fa, tags), Style::default().fg(app.theme.blue())),
            ]),
        ];
        ...
        let crypto_lines = vec![
            Line::from(vec![
                Span::styled("Crypto", Style::default().fg(app.theme.red()).add_modifier(Modifier::BOLD)),
            ]),
            Line::from(""),
            Line::from(vec![
            ...
            Line::from(vec![
                Span::styled("│  ", Style::default().fg(app.theme.gray())),
                Span::styled(if is_aes { "AES-NI" } else { "ChaCha" }, Style::default().fg(app.theme.green()).add_modifier(Modifier::BOLD)),
                Span::styled("  │", Style::default().fg(app.theme.gray())),
            ]),
            Line::from(vec![
                Span::styled("└──────────┘", Style::default().fg(app.theme.gray())),
            ...
            Line::from(vec![
                Span::styled("Hardware", Style::default().fg(if is_aes { app.theme.green() } else { app.theme.red() })),
            ]),
        ];
    ...
    let menu_block = Block::default()
        ...
        .title(Span::styled(" [ Menu ] ", Style::default().fg(app.theme.green()).add_modifier(Modifier::BOLD)));
    ...
    if let Some(ref vault) = app.vault {
        ...
        let with_notes = vault.e.iter().filter(|e| e.nt.as_ref().map_or(false, |n| !n.is_empty())).count();
        
        let two_fa_pct = if total > 0 { (with_2fa * 100) / total } else { 0 };
        let url_pct = if total > 0 { (with_url * 100) / total } else { 0 };
        let notes_pct = if total > 0 { (with_notes * 100) / total } else { 0 };
        ...
        let mut stats_lines = vec![
            Line::from(vec![
                Span::styled("Password Statistics", Style::default().fg(app.theme.yellow()).add_modifier(Modifier::BOLD)),
            ]),
            Line::from(""),
            Line::from(vec![
                Span::styled("━━━━━━━━━━━━━━━━━━━━━━━", Style::default().fg(app.theme.gray())),
            ...
            Line::from(""),
            Line::from(vec![
                Span::styled("Total Entries", Style::default().fg(app.theme.gray())),
            ]),
            Line::from(vec![
                Span::styled(format!("   {}", total), Style::default().fg(app.theme.yellow()).add_modifier(Modifier::BOLD)),
            ]),
            Line::from(""),
            Line::from(vec![
                Span::styled("2FA Protection", Style::default().fg(app.theme.gray())),
            ]),
            Line::from(vec![
                Span::styled(format!("   {}", create_bar(two_fa_pct)), Style::default().fg(app.theme.green())),
            ]),
            Line::from(vec![
                Span::styled(format!("   {} entries ({}%)", with_2fa, two_fa_pct), Style::default().fg(app.theme.green())),
            ]),
            Line::from(""),
            Line::from(vec![
                Span::styled("With URLs", Style::default().fg(app.theme.gray())),
            ]),
            Line::from(vec![
                Span::styled(format!("   {}", create_bar(url_pct)), Style::default().fg(app.theme.blue())),
            ]),
            Line::from(vec![
                Span::styled(format!("   {} entries ({}%)", with_url, url_pct), Style::default().fg(app.theme.blue())),
            ]),
            Line::from(""),
            Line::from(vec![
                Span::styled("With Notes", Style::default().fg(app.theme.gray())),
            ]),
            Line::from(vec![
                Span::styled(format!("   {}", create_bar(notes_pct)), Style::default().fg(app.theme.purple())),
            ]),
            Line::from(vec![
                Span::styled(format!("   {} entries ({}%)", with_notes, notes_pct), Style::default().fg(app.theme.purple())),
            ]),
        ];
        ...
        if !app.all_tags.is_empty() {
            ...
            stats_lines.push(Line::from(""));
            stats_lines.push(Line::from(vec![
                Span::styled("Top Tags", Style::default().fg(app.theme.aqua()).add_modifier(Modifier::BOLD)),
            ]));
            
            for (tag, count) in app.all_tags.iter().take(3) {
                stats_lines.push(Line::from(vec![
                    Span::styled(format!("   {} ", tag), Style::default().fg(app.theme.purple())),
